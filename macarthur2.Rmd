...
fnames <-paste0("raw/macfound.org/", janitor::make_clean_names(fellows$name),".html") 
names(fnames) <- fellows$name   

length(fnames)

## Get every webpage, dropping the safely() error codes and NULL entries
...
p +   geom_boxplot(outlier.colour = NA, mapping = aes(group = year)) + 
  geom_jitter(alpha = 0.4, shape = 1, position = position_jitter(height = 0.2)) + 
